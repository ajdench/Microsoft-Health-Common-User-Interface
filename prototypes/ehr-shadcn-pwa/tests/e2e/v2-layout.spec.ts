...
test('renders shadcn-native V2 consultation shell without horizontal overflow', async ({ page }) => {
  ...
  await expect(reason.getByText('Hypertensive disorder [38341003]')).toBeVisible()
  ...
  await expect(reason.getByText('Type 2 diabetes mellitus [44054006]')).toBeVisible()
  ...
  await expect(reason.getByText('Type 2 diabetes mellitus [44054006]')).toHaveCount(0)
