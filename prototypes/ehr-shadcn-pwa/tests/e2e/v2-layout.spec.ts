...
test('renders shadcn-native V2 consultation shell without horizontal overflow', async ({ page }) => {
  ...
  await expect(consultation.getByRole('list', { name: 'Tasks' })).toHaveCount(0)
  ...
  await consultation.getByRole('button', { name: 'Add task' }).click()
  await expect(consultation.locator('[data-tasks-pane-status]')).toHaveText('Optional')
  await expect(consultation.getByRole('list', { name: 'Tasks' }).getByText('Arrange diabetic foot review')).toBeVisible()
