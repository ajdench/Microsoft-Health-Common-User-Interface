...
| Tasks | Bottom shadcn `Card` using the same split body as consultation sections, with task entry on the left and task-local SNOMED CT coding on the right |
...
follow-up work. Tasks also use the same split-card rail and SNOMED CT concept
search/selected concept pill pattern as consultation sections, but the left
side is task entry rather than narrative note text and the resulting codes are
scoped to task-owned entries rather than a narrative section.
