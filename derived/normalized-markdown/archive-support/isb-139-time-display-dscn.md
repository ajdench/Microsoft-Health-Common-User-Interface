# isb-139-time-display-dscn

## Provenance
- Source file: `raw/sources/references/nhs-archives/isb-standards/dscn-pdfs/isb-139-time-display-dscn.pdf`
- Extracted text: `derived/extracted-text/archive-support/isb-139-time-display-dscn.txt`
...
Sponsoring Organisation:                                Implementation Date:   01 December 2015
                                                        Subject:
    DH Informatics                                      Time Display for Clinical Systems
...
Summary:
This user interface standard defines the valid display values for exact time, approximate time, time
duration and display of date and time combinations.

Organisations developing or upgrading applications for which the standard is in scope MAY
implement the time display standard immediately and SHOULD do so at their next major system
release. All clinical systems within the NHS in England MUST conform to the time display standard
by 01 December 2015.

Patient safety risks associated with the time display standard are specified in the safety case
...
Reference No:                    DSCN 08/2010
Version No:                      1.0
Subject:                         Time Display for Clinical Systems within the NHS in England
Type of Change:                  Introduction of a new approved Information Standard
Implementation Date:             01 December 2015
Business Justification:          To make display and user interaction consistent across all
                                 clinical systems within the NHS in England in order to increase
                                 patient safety by maximising clinical utility and minimising
                                 reading and transcribing errors.
 ...
 Background
 The display of exact times, approximate times and durations within software applications has inherent
 risks of misinterpretation based on how these values are displayed. Currently, there is no specific
 time display standard within the NHS for clinical applications, implying that various systems across
 the NHS will implement different approaches to displaying time. Because of this inconsistency, risks
 to effective clinical care and to patient safety arise.

 Currently, clinical and healthcare IT systems used across all care settings in the NHS in England
 differ in the way that time is displayed. For example, our review of clinical systems showed a mix of
 both 24-hour and 12-hour time display formats being used. Inherent within this is the risk that
 healthcare professionals moving between clinical systems made by different suppliers can
 misinterpret time, leading to patient safety incidents, as defined by the National Patient Safety
 Agency (NPSA). This risk is particularly relevant to healthcare practitioners who trained outside of the
 UK, since they may well be used to different conventions.

 The safety case and guidance accompanying the standard provide evidence of clinical safety issues
 and there is a clear argument that unambiguous time display would reduce the potential for human
 error leading to patient safety incidents.
...
 The following items are covered in the guidance:
 ...
 Safety Case
 Clinical safety requirements and objectives related to the display of times in clinical systems are
...
 Effects on Other Information Standards:
 ...
 Further Information and Support
        CUI Time Display Design Guide - www.cui.nhs.uk
